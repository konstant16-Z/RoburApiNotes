...
## Стойка знака на поперечнике — тот же write-элемент `DwgRoadSignStand`, паттерн «знак на пикете поперечника»

`[DECOMP]`-мост (typedef row 16, уже в §«Стойка знака»): стойка ставится не только
на пикет ПЛАНА, но и на пикет ПОПЕРЕЧНИКА — тем же паттерном «знак на пикете»
(`baseinspos` + смещение + угол, см. §«Стойка знака» выше). Разница только в том,
**какой пикет** берётся: для поперечника — пикет `Section.Station` из
`road.md` §«Поперечники» (`Corridor.Sections`, `SectionList.Add(station)`), т.е.
стойка стоит В СЕЧЕНИИ поперечника на его пикете, write-коллекция та же
(`DwgRoadSignStand`), a НЕ отдельная поперечная сущность и НЕ табличка (§ ниже).

Это мост `roadsigns.md` §«Стойка знака» ↔ `road.md` §«Поперечники»: write-факты
стойки (`[TUT]` 68 страниц, `[DECOMP]` typedef row 16) переносятся на поперечник
без изменения набора write-имён — меняется только выбор пикета (`Section.Station`).
