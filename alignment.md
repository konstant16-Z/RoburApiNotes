...
- `VertexItem` — **структура** с публичными полями `L1, R, K, L2`; при чтении `L2[i] == L1[i+1]` (цепочка), хвостовой L2 — отдельное поле вершины. Копировать-изменять-записывать обратно (`vertex[j] = item`).
...
`(parent, name, description, color, showDifference)` — поля после этого менять можно.
...
(индекс найденной секции либо позицию вставки); новая секция создаётся с
...
**Статическая земля секции.** У `Section` свойствa `StaticEg`/`SectionLine` имеют
...
`_build_recipe` (ручная сборка из `components` JSON) остаётся фоллбеком на случай
экспорта без `.act`.
