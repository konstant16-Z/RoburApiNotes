...
| `cadView.ZoomBound(box, true)` | Зум на рамку `BoundingBox2D` (см. `geometry.md`) | `[CODE]` Runoff |
...
| API | Назначение | Статус |
|---|---|---|
| `System.Windows.Forms.FolderBrowserDialog` | Графический выбор папки (WinForms, .NET 4.8; команды Robur — UI-поток, STA) | `[CODE]` SurfaceIO (прогон подтверждён); RailModelExporter (IronPython 2.6) |

```csharp
// C# (проверено в плагине SurfaceIO)
...
Вариант с **опциями** (`[CODE]` robur-mcp) — третьим аргументом
...
| `CadPen.DrawArray(positions, ArrayMode.Polyline)` | Нарисовать контур по точкам (`positions` — `IEnumerable<Vector2D>`; `[CODE]` robur-mcp) | `[CODE]` |
